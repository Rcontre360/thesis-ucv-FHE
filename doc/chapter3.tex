...
En cuanto a los parámetros del esquema, los valores que el usuario indica son los tres siguientes. Los dos primeros se pasan al método \textit{generate\_config} del modelo secuencial, que deriva el resto de la configuración a partir de la red ya construida; el tercero se fija sobre el objeto de configuración resultante.
...
El objeto de configuración resultante es una estructura de Python cuyos campos permanecen accesibles, de modo que un usuario con conocimientos del esquema puede ajustarlos antes de construir el contexto. Los campos generados por la biblioteca son los siguientes:
...
Cada asignación hecha por el usuario dispara la validaciones descritas más adelante, por lo que una configuración inconsistente se rechaza en el momento de escribirla y no al invocar al \textit{backend}.

\subsubsection{Validaciones de configuración}
...
\subsubsection{Módulos e interfaz pública}\label{sec:metodo_api}

La biblioteca se distribuye como el paquete \nolinkurl{fhe_ml}, organizado en los módulos que se resumen en la \hyperref[tab:metodo_modulos]{Tabla~\ref{tab:metodo_modulos}}. El código fuente y la documentación de uso están disponibles en el repositorio público del proyecto \cite{SDKRepo}.

\begin{table}[h]
    \centering
    \caption{Módulos del paquete \nolinkurl{fhe_ml}.}\label{tab:metodo_modulos}
    \vspace{0.5em}
    \begin{tabular}{| l | p{8.2cm} |}
        \hline
        Módulo & Contenido \\
        \hline
        \repofile{src/fhe_ml/backend} & Enlaces con HEonGPU construidos con pybind11. \\
        \hline
        \repofile{src/fhe_ml/ckks} & Contexto criptográfico, configuración del esquema, validaciones y los contenedores de datos cifrados y en texto plano. \\
        \hline
        \repofile{src/fhe_ml/layers} & Capas de red neuronal: \textit{Linear}, \textit{Conv2D}, \textit{ReLU}, \textit{Square} y la entrada del modelo. \\
        \hline
        \repofile{src/fhe_ml/sequential.py} & Clase \textit{Sequential}, que compone las capas y orquesta la compilación y la inferencia. \\
        \hline
        \repofile{src/fhe_ml/client.py} & Clase \textit{Client}, que posee la clave secreta y realiza el cifrado y el descifrado. \\
        \hline
        \repofile{src/fhe_ml/utils} & Conversión de datos, validación de formas y errores propios de la biblioteca. \\
        \hline
    \end{tabular}
\end{table}

Las dependencias entre estos módulos se muestran en la \hyperref[fig:metodo_modulos]{Figura~\ref{fig:metodo_modulos}}. Las flechas indican qué módulo utiliza a cuál: el flujo va del modelo secuencial hacia las capas y el cliente, de estos hacia el contexto y los contenedores de datos, y de todos ellos hacia el \textit{backend}, que es el único punto por el que la biblioteca alcanza a HEonGPU.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{images/modulos_dependencias.png}
    \caption{Dependencias entre los módulos del paquete \nolinkurl{fhe_ml}.}
    \label{fig:metodo_modulos}
\end{figure}

La interfaz pública que el usuario importa se reduce a un conjunto reducido de nombres: el contexto y su configuración (\textit{FHEContext}, \textit{FHEConfig} y \textit{BootstrapConfig}), el modelo y sus capas (\textit{Sequential}, \textit{Linear}, \textit{Conv2D}, \textit{ReLU} y \textit{Square}), el cliente (\textit{Client}), los contenedores de datos (\textit{EncryptedVector}, \textit{PlaintextVector} y \textit{PlaintextTensor}) y los errores que la biblioteca puede arrojar. Ninguna clave, ni el nivel de los textos cifrados, ni los objetos del \textit{backend} forman parte de esta interfaz.

El \hyperref[lst:metodo_api]{Código~\ref{lst:metodo_api}} muestra el uso completo de la biblioteca sobre una red de dos capas, siguiendo los cuatro pasos descritos anteriormente.

\begin{lstlisting}[language=Python,caption={Inferencia cifrada de una red de dos capas.},label={lst:metodo_api}]
import numpy as np
from fhe_ml import Client, FHEContext, Sequential
from fhe_ml.layers import Linear, ReLU

# 1. Construccion del modelo con los pesos ya entrenados.
model = Sequential([
...
# 2. Configuracion del esquema y compilacion contra el contexto.
#    La calibracion de los rangos ocurre dentro de compile().
config = model.generate_config(scale=40, relu_degrees=(3,))
context = FHEContext(config)
model.compile(context, datos_de_calibracion)

# 3. El cliente genera las claves, cifra la entrada y la red se evalua.
client = Client(context)
context.set_client_params(client.key_params())
salida_cifrada = model(client.encrypt(entrada))

# 4. Descifrado del resultado.
resultado = client.decrypt(salida_cifrada)
\end{lstlisting}

La configuración del esquema no se escribe a mano: el método \textit{generate\_config} la deriva del propio modelo, a partir del factor de escala y de los grados de los polinomios de la activación, calculando la cadena de módulos necesaria y activando el \textit{bootstrapping} si la profundidad multiplicativa lo exige. Como alternativa a enumerar las capas, el método \textit{Sequential.from\_torch} construye el modelo equivalente a partir de una red ya entrenada en PyTorch y la forma de una muestra de entrada.

\subsubsection{Clase \textit{Sequential}}
...
Antes de poder inferir, el modelo debe llamar la funcion \textit{compile} contra un contexto criptográfico; este paso, que entre otras tareas calibra la red, se describe más adelante en \hyperref[sec:metodo_calibracion]{Calibración de la red}. La clase admite además construir la red enumerando las capas manualmente o, alternativamente, a partir de un modelo entrenado en PyTorch, traduciendo automáticamente las capas soportadas.
