% --- Part 4: Redes Neuronales y Cifrado Homomórfico ---
\section{Redes Neuronales y Cifrado Homomórfico}
\SectionFrame{Redes Neuronales y Cifrado Homomórfico}

\begin{frame}{Operaciones en el espacio cifrado}
    La ejecución de RNAs sobre FHE requiere traducir las operaciones tensoriales al dominio de polinomios:
    \vspace{0.4cm}
    \begin{itemize}
        \setlength{\itemsep}{12pt}
        \item \textbf{Multiplicación Matriz-Vector:} Se optimiza mediante la diagonalización de pesos, minimizando las rotaciones homomórficas costosas.
        \item \textbf{Convoluciones:} Se transforman en productos matriciales eficientes mediante la técnica \textbf{Im2Col} sobre los datos cifrados.
        \item \textbf{Funciones de Activación:} Dado que FHE solo permite sumas y productos, activaciones como ReLU o Sigmoid se sustituyen por \textbf{aproximaciones polinomiales} (ej. $f(x) = x^2$).
    \end{itemize}
\end{frame}

\begin{frame}{Redes de interés}
    \begin{itemize}
        \item \textbf{Capa Lineal ($y = Wx + b$):} Base de los Perceptrones Multicapa (MLP), resolvemos mediante productos punto paralelos.
        \item \textbf{Capa Convolucional:} Fundamental para visión artificial, procesa regiones espaciales locales.
    \end{itemize}
    \vfill
    \centering
    \begin{tikzpicture}[scale=0.6, every node/.style={font=\tiny}]
        % ENTRADA
        \begin{scope}[shift={(0,0)}]
            \draw[step=0.7cm, gray!60, thin] (0,0) grid (2.1,2.1);
            \fill[blue!20, opacity=0.5] (0,0.7) rectangle (1.4,2.1);
            \node at (0.35, 1.75) {$x_1$}; \node at (1.05, 1.75) {$x_2$};
            \node at (0.35, 1.05) {$x_4$}; \node at (1.05, 1.05) {$x_5$};
            \node[below] at (1.05, 0) {Entrada};
        \end{scope}
        % KERNEL
        \begin{scope}[shift={(3.5,0.35)}]
            \draw[step=0.7cm, gray!60, thin] (0,0) grid (1.4,1.4);
            \fill[orange!20, opacity=0.5] (0,0) rectangle (1.4,1.4);
            \node at (0.35, 1.05) {$w_1$}; \node at (1.05, 1.05) {$w_2$};
            \node at (0.35, 0.35) {$w_3$}; \node at (1.05, 0.35) {$w_4$};
            \node[below] at (0.7, 0) {Kernel};
        \end{scope}
        \draw[->, thick, gray!60] (2.2, 1.05) -- (3.4, 1.05);
        \draw[->, thick, gray!60] (5.0, 1.05) -- (6.2, 1.05);
        % SALIDA
        \begin{scope}[shift={(6.5,0.7)}]
            \draw[step=0.7cm, gray!60, thin] (0,0) grid (0.7,0.7);
            \fill[green!20] (0,0) rectangle (0.7,0.7);
            \node at (0.35, 0.35) {$z_{1,1}$};
            \node[below] at (0.35, 0) {Salida};
        \end{scope}
    \end{tikzpicture}
\end{frame}
