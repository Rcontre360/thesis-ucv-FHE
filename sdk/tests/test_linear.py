...
class TestLinear:
    def test_identity_matrix_passthrough(self, built_context, client):
        W = [[1.0, 0.0], [0.0, 1.0]]
        layer = Linear(2, 2, W)
        ...
        result = client.decrypt(layer(ct))[:2]
        assert abs(result[0] - 0.3) < EPSILON
        assert abs(result[1] - (-0.7)) < EPSILON

    def test_output_size(self, built_context, client):
    ...
    def test_scale_row(self, built_context, client):
        W = [[2.0, 0.0], [0.0, 3.0]]
        layer = Linear(2, 2, W)
        layer._weight.encode(built_context)
        client.generate_rotation_keys(layer.bsgs_shifts())
        built_context.set_client_params(client.key_params())
        ct = client.encrypt([0.5, 0.4])
        result = client.decrypt(layer(ct))[:2]
        assert abs(result[0] - 1.0) < EPSILON
        assert abs(result[1] - 1.2) < EPSILON

    def test_bias_added(self, built_context, client):
        W = [[1.0, 0.0], [0.0, 1.0]]
        b = [0.1, -0.2]
