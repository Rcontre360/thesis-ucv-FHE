...
class TestReLU:
    def test_encrypted_far_from_kink_positive(self, client):
        ...
        relu = _relu((3,))
        ct = client.encrypt([1.0])
        result = client.decrypt(relu(ct))[0]
        assert abs(result - 1.0) < 0.2

    def test_encrypted_far_from_kink_negative(self, client):
        relu = _relu((3,))
        ct = client.encrypt([-1.0])
        result = client.decrypt(relu(ct))[0]
        assert abs(result - 0.0) < 0.2

    def test_batch_output_size(self, client):
        relu = _relu((3,))
        ct = client.encrypt([0.1, -0.3, 0.7, -0.9])
