...
def _fn_coeffs(degree: int) -> np.ndarray:
    """Cheon `f_n` of degree 2n+1: the unique odd polynomial with f(1)=1 and
    f^(j)(1)=0 for j=1..n (ePrint 2019/417)."""
...
class ReLU(Layer):
    ...
    def __init__(self) -> None:
        self._degrees: tuple[int, ...] | None = None
        self._coeffs: list[np.ndarray] | None = None
    ...
    def __call__(self, x: EncryptedVector) -> EncryptedVector:
        ...
        ctx = x.context
        # Composed f_n chain pushes inputs toward sign(x) (±1 in the working range).
        sign_approx = x
        for poly_coeffs in self._coeffs:
            # f_n of degree d burns (d+3)/2 levels: 1 for x², (d-3)/2 Horner
            # mults, 1 final x·q(x²).
        ...
        # Smooth ReLU(x) = x · (sign(x)+1)/2; the +0.5 folded into the last poly.
    ...
    def _eval_odd_poly(x: EncryptedVector, coeffs: np.ndarray) -> EncryptedVector:
        """Evaluate p(x) = c0 + x · q(x²); q's coefficients are the odd-indexed
        entries of `coeffs`."""
        # c0 is 0 for inner f_n steps; only the final smooth-ReLU layer carries 0.5.
        ...
        # x is untouched since entry; drop it to align with q(x²)'s levels.
    ...
    def _horner_on_x_squared(
        ...
        """Compute q(x²) via Horner. Saves depth vs. Horner on x: deg(q) mults,
        not 2·deg(q)."""
