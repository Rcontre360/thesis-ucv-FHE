...
from bench.shared.metrics import r2_score
...
def run(case_dir: str) -> None:
    ...
    with Measure(alloc_probe=alloc_probe) as m_infer:
        with torch.no_grad():
            _ = model(torch.tensor(x_lat, dtype=torch.float32, device=device)).cpu().numpy().reshape(-1)

    with torch.no_grad(), Timer() as t_acc:
        acc_preds = model(torch.tensor(x_acc, dtype=torch.float32, device=device)).cpu().numpy().reshape(-1)
    r2 = r2_score(y_acc, acc_preds)
