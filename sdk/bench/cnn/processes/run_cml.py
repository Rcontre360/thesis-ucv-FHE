...
def run(case_dir: str) -> None:
    ...
    calib = torch.tensor(data["x_calib"], dtype=torch.float32).reshape(-1, CHANNELS, h, w)
    ...
    with Measure() as m_infer:
        for i, x in enumerate(x_lat):
            img = x.reshape(1, CHANNELS, h, w)
            enc_logits[i] = cml_model.forward(img, fhe="execute").reshape(-1)[:N_CLASSES]

    with Timer() as t_acc:
        acc_logits = np.stack([
            cml_model.forward(x.reshape(1, CHANNELS, h, w), fhe="disable").reshape(-1)[:N_CLASSES]
            for x in x_acc
        ])
