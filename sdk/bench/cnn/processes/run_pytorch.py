...
from bench.cnn.model import build_network, CHANNELS, IMAGE_SHAPE
...
def run(case_dir: str) -> None:
    ...
    with Measure(alloc_probe=alloc_probe) as m_infer:
        with torch.no_grad():
            x = torch.tensor(x_lat, dtype=torch.float32, device=device).reshape(-1, CHANNELS, *IMAGE_SHAPE)
            _ = model(x).cpu().numpy()

    with torch.no_grad(), Timer() as t_acc:
        x = torch.tensor(x_acc, dtype=torch.float32, device=device).reshape(-1, CHANNELS, *IMAGE_SHAPE)
        acc_logits = model(x).cpu().numpy()
